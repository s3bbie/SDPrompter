...
import {
  ChevronLeft, Settings, X, RotateCcw, Pencil, Check, Trash2
} from "lucide-react";
...
export default function TeleprompterPlayer({ script, onExit, onSave, onDelete }) {
  ...
  const [editing, setEditing] = useState(!script?.content); // auto-open edit if new/empty
  ...
  const handleStartStop = () => {
    ...
    setOffset(0); // always start from top
  ...
  const btnLabel = isCountingDown ? "CANCEL" : scrolling ? "STOP" : "START";
  const btnBg    = isCountingDown ? "#d97706" : scrolling ? "#dc2626" : "#2563eb";

  return (
    <div
      className="fixed inset-0 flex flex-col z-50"
      style={{ background: editing ? "#0f0f0f" : bgColor }}
    >
      {/* ── TOP BAR ── */}
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "12px 16px 10px",
        borderBottom: `1px solid ${editing ? "#2a2a2a" : "rgba(255,255,255,0.08)"}`,
        gap: 10,
      }}>
        {/* Back */}
        <button
          ...
          style={{
            ...
            cursor: "pointer", fontSize: 15, fontWeight: 500, flexShrink: 0,
          }}
        >
          <ChevronLeft size={20} style={{ marginLeft: -4 }} />
          Scripts
        </button>
        ...
        <div style={{ flex: 1, textAlign: "center" }}>
          {editingTitle ? (
            <input
              ...
              style={{
                background: "none", border: "none", borderBottom: "1px solid #3a3a3c",
                color: "#fff", fontSize: 15, fontWeight: 600,
                textAlign: "center", outline: "none", width: "100%", padding: "2px 4px",
              }}
              autoFocus
            />
          ) : (
        ...
        {editing ? (
          <button
            ...
            style={{
              background: "none", border: "none", color: "#2563eb",
              cursor: "pointer", fontSize: 15, fontWeight: 600, flexShrink: 0,
            }}
          >
            Done
          </button>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <button
              onClick={() => { setScrolling(false); setCountdown(null); setEditing(true); }}
      ...
      {editing ? (
        ...
        <textarea
          ...
          style={{
            flex: 1, width: "100%", background: "#0f0f0f", color: "#fff",
            fontSize: 20, lineHeight: 1.7, padding: "20px 20px",
        ...
        >
          ...
          >
            {text || (
              <span style={{ color: "#444", fontStyle: "italic" }}>
                Tap the pencil icon to add your script…
              </span>
            )}
          </div>
          ...
          {isCountingDown && (
            <div style={{
              ...
              <span style={{ color: "#fff", fontWeight: 800, fontSize: "22vw" }}>
          ...
          {!scrolling && !isCountingDown && offset !== 0 && (
            <div style={{
              ...
              fontSize: 12, padding: "4px 14px", borderRadius: 20, pointerEvents: "none",
            }}>
              Tap to resume
            </div>
          )}
      ...
      {/* ── BOTTOM CONTROLS (prompter mode only) ── */}
      {!editing && (
        <div
          style={{
            display: "flex", alignItems: "center", justifyContent: "space-between",
            padding: "12px 16px 20px",
            background: `color-mix(in srgb, ${bgColor} 85%, #000 15%)`,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <button
              ...
              style={{
                ...
                color: mirror ? "#2563eb" : "#555", fontSize: 20, fontWeight: 700,
              }}
              title="Mirror"
            >⇔</button>
            <button
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "4px 20px 16px" }}>
              ...
              <button onClick={() => setShowSettings(false)} style={{ background: "none", border: "none", color: "#666", cursor: "pointer" }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
                  {FONT_COLORS.map(c => (
                    <ColorSwatch key={c} color={c} selected={fontColor === c} onClick={() => setFontColor(c)} />
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 10, flexWrap: "wrap" }}>
                  {BG_COLORS.map(c => (
                    <ColorSwatch key={c} color={c} selected={bgColor === c} onClick={() => setBgColor(c)} showBorder />
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  {[0,3,5,10].map(s => (
                    <ToggleBtn key={s} active={countdownSecs === s} onClick={() => setCountdownSecs(s)}>
                      {s === 0 ? "Off" : `${s}s`}
                    </ToggleBtn>
                  ))}
                </div>
              ...
              <div style={{ borderTop: "1px solid #2a2a2a", paddingTop: 20, marginTop: 4 }}>
                <button
                  onClick={() => {
                    if (confirm(`Delete "${title}"?`)) { onDelete(script.id); }
                  }}
                  style={{
...
function SectionLabel({ children }) {
  return (
    <p style={{ color: "#888", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, margin: 0 }}>
      {children}
    </p>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${active ? "#2563eb" : "#3a3a3c"}`,
  ...
  return (
    <button
      ...
      style={{
        ...
        border: selected ? "2px solid #2563eb" : showBorder ? "1px solid #3a3a3c" : "2px solid transparent",
        cursor: "pointer", outline: selected ? "2px solid #2563eb" : "none",
  ...
  return (
    <button
      ...
      style={{
        ...
        background: on ? "#2563eb" : "#3a3a3c", cursor: "pointer",
